...
def create_qr_code(doc, method=None):
    ...
    if qr_code and frappe.db.exists({"doctype": "File", "file_url": qr_code}):
        qr = frappe.get_doc("File",{"file_url": qr_code})
        frappe.delete_doc("File",qr.name)
    ...
    if "ksa_einv_qr" in [d.fieldname for d in meta.get_image_fields()]:
        ...
        # Invoice Amount
        print("\n\n\n\n\ndoc.grand_total",doc.grand_total)
        print("\n\n\n\n\ndoc.grand_total",format(doc.grand_total, '.2f'))
        invoice_amount = str(format(doc.grand_total, '.2f'))
        # invoice_amount = str(doc.grand_total)
        print("\n\n\n\n\ninvoice_amount",invoice_amount)
        tag = bytes([4]).hex()
        print("\n\n\n\n\nbytes([4]),tag",bytes([4]),tag)
